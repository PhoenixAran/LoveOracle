    ...
    if self.animViewerCurrentSource == AnimationSource.Builder then
      if lume.count(self.animViewerBuilderSelect) > 0 then
          ...
          for k, v in ipairs(self.animViewerBuilderSelect) do
            ...
            if imgui.Selectable(self.animViewerBuilderSelect[k], isSelected) then
              ...
              if lume.any(self.animViewerAnimationSelect) then
                self.animViewerCurrentAnimation = self.animViewerCurrentBuilder.animations
                [lume.first(self.animViewerAnimationSelect)]
              end
        ...
        if self.animViewerCurrentBuilder.type == 'animated_sprite_renderer' then
          ...
          imgui.Separator()
          imgui.Image(self.animViewerCanvas, self.animViewerCanvas:getWidth(), self.animViewerCanvas:getHeight())
          assert(self.animViewerCurrentAnimation ~= nil and self.animViewerCurrentAnimation:getSpriteFrames(),
            'Expected an animation instance. Did you forget to set a default substrip?')
          local frameIndex = imgui.SliderInt('Frame', self.animViewerFrameIndex, 1,
            lume.count(self.animViewerCurrentAnimation:getSpriteFrames()))
...
function ContentViewer:draw()
  ...
  if self.animViewerDrawCenterPoint then
    ...
    love.graphics.line(animCanvasCenterX, animCanvasCenterX - (crosshairLen / 2), animCanvasCenterX,
      animCanvasCenterY + (crosshairLen / 2))
    love.graphics.line(animCanvasCenterX - (crosshairLen / 2), animCanvasCenterY, animCanvasCenterX + (crosshairLen / 2),
      animCanvasCenterY)
  end
